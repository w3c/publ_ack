...
async function create_ack_section(config) {
    ...
    try {
        ...
        console.log(final_current);

        const retval = config.template.replace(EXPLICIT_LISTING, generate_list(final_separate_acks)).replace(MEMBER_LISTING, generate_list(final_current));
        console.log(retval);
    } catch (err) {
