...
async function get_configuration() {
    ...
    */
    ...
    const final_documents = { ...user_config.documents, ...file_config.documents };
    const final_config    = { ...user_config, ...file_config };
    ...
    if (Object.keys(final_config).length > 0) {
        ...
        if (!check_final_config(final_config, group)) {
            ...
            document_config.list = document_config.list.filter((person) => person.name);
